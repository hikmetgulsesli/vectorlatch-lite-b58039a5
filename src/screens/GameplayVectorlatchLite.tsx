...
export type GameplayVectorlatchLiteActionId = "act-pause-game-1" | "act-start-game-2" | "act-restart-game-3";

export interface GameplayVectorlatchLiteProps {
  actions?: Partial<Record<GameplayVectorlatchLiteActionId, () => void>>;
  runtime?: { player?: { lane?: number; position?: number }; obstacles?: Array<{ lane?: number; position?: number }>; shards?: Array<{ lane?: number; position?: number }>; score?: number; energy?: number; lives?: number; paused?: boolean; level?: number; gameOver?: boolean };
...
export function GameplayVectorlatchLite({ actions, runtime }: GameplayVectorlatchLiteProps) {
  ...
  const gameOver = runtime?.gameOver ?? false;
  const showStartOverlay = paused || gameOver;

  const pad = (n: number, len = 5) => String(n).padStart(len, '0');
  const laneLeft = playerLane === 0 ? '20%' : playerLane === 2 ? '80%' : '50%';
  ...
  return (
    <>
      ...
      <div className="font-display-score text-display-score text-primary glow-cyan">{pad(score)}</div>
      </div>
      ...
      <div className="font-data-point text-data-point text-secondary-container">{pad(level, 2)}</div>
      </div>
      </div>
      ...
      <span className="text-tertiary-container">{Math.round(energy)}%</span>
      </div>
      <div className="h-2 bg-surface-container-high rounded-full overflow-hidden w-full relative">
      <div className="absolute top-0 left-0 h-full bg-tertiary-container shadow-[0_0_10px_rgba(255,201,244,0.5)]" style={{ width: `${energy}%` }}></div>
      </div>
      </div>
      {/* Restart Button */}
      <button aria-label="ACT_RESTART_GAME" className="bg-surface/70 backdrop-blur-md border border-primary/20 p-3 rounded-DEFAULT hover:bg-primary/10 transition-colors text-primary" type="button" data-action-id="act-restart-game-3" onClick={actions?.["act-restart-game-3"]}>
      ...
      {/* The Latch (Magenta) */}
      <div className="absolute top-1/2 -translate-y-1/2 w-8 h-12 md:w-12 md:h-16 border-2 border-[#ff00ff] bg-[#ff00ff]/20 glow-magenta latch-animation flex flex-col justify-between items-center py-1" style={{ left: laneLeft, transform: 'translate(-50%, -50%)' }}>
      ...
      {/* Pre-game / Start Overlay (shown when paused or game over) */}
      <div className={`absolute inset-0 z-40 bg-surface-container-lowest/80 backdrop-blur-sm flex items-center justify-center flex-col gap-8 ${showStartOverlay ? 'flex' : 'hidden'}`} id="startScreen">
